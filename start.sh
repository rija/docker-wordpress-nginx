...
if [ ! -f /usr/share/nginx/www/wp-config.php ]; then
  ...
  sed -e "s/'database_name_here'/getenv('DB_ENV_MYSQL_DATABASE')/
  s/'localhost'/getenv('DB_PORT_3306_TCP_ADDR')/
  s/'username_here'/getenv('DB_ENV_MYSQL_USER')/
  s/'password_here'/getenv('DB_ENV_MYSQL_PASSWORD')/
